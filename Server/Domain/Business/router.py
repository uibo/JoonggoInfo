...
def cal_mvl_data(model: str|None = None, storage: str|None = None,
                    ...
                    feature_list: str = "0000000", search_date: str = '2022-01-012024-11-30', db: Session=Depends(get_db)):
    ...
    if (bool(records)):
        ...
        mal_dict = dict()
        volume_dict = dict()
        for i in range(9, df.index.size):
            each_date = df.iloc[i].name.strftime("%Y-%m-%d")
            volume = int(df.iloc[i, 1])
            volume_dict[each_date] = volume
            days10_price_sum = int(df.iloc[i-9:i+1, 0].sum())
            days10_count_sum = int(df.iloc[i-9:i+1, 1].sum())
            mal_dict[each_date] = days10_price_sum//days10_count_sum if days10_price_sum else 0
